...
public class Request {
    ...
    internal init(urlRequest: URLRequest, httpMethod: HTTPMethod) {
        ...
        self.urlRequest.httpMethod = httpMethod.rawValue
    ...
    public func set(headers: [String: String])-> Request {
        ...
        urlRequest = RequestBuilder.setupURLRequest(url: urlRequest.url!, httpMethod: httpMethod, headers: headers)
        return self
    }
    
    public func set(urlParams: [String: Any])-> Request {
    ...
    public func fire<T: Decodable>(completion: @escaping(Result<(T, Int?), JetError>)-> Void) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            self.dataTask = JetRequest.urlSession.dataTask(with: self.urlRequest) { data, res, error in
                let statusCode = (res as? HTTPURLResponse)?.statusCode
                DispatchQueue.main.async {
                    guard 200 ... 299 ~= (statusCode ?? 0), error == nil, data != nil else {
